...
![Spread in ticks](fig/figure_1.png){#id .class width=39% height=39%}
...
![Spread profile](fig/figure_2.png){#id .class width=39% height=39%}

From the spread profile of 0050, we can observe that in the first 5 minutes of market open, the spread tends to be significantly larger than the ones in other period of a day. We believe it is due to the active trading activity at market open since many traders are using some ETF tracking or arbitrage strategies. Since there are more pricing and flows during the first minutes of market open, spread may occurs in this perid and there are more chances for us to utilize it with our proposed order placement strategy.
...
- Meanwhile, we define $t_\star$ as the maximum time we want to wait until the order gets hit or we cross the spread to buy at the ask price 1 which denoted as $a_1$. Besides, we define $t_m$ as the mid time when we decide to replace our bid order to the mid quote $b_\star$ expecting the $a_1$ will comes down so that we can benefit our pnl from the price movement. 
...
- After Replacing the order at $b_1$ with $b_\star$ at $t_m$, we waited til $t_\star$ and no people hit our price, we then cross the spread, fill at $a_1$

![Siulation Score Mean](fig/figure_3.png){#id .class width=99% height=99%}

In this above example, we enter the trade at 146 when the ask is 146.15 and bid sits at 146. After 5000ms($t_m$), our order at $b_1$ has not been executed, we then lift it to 146.5 which is one tick higher than the original price. However, we didn't get filled after $t_m$ so we cross the spread and take the price at $a_1$. 
...
![Siulation Score Mean](fig/figure_9.png){#id .class width=99% height=99%}
...
![Cases Distribution Breakdown](fig/figure_10.png){#id .class width=70% height=70%}
